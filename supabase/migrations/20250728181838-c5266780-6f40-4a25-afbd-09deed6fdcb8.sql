...
ON accommodation_types FOR SELECT USING (is_active = true);

CREATE POLICY "Админы и менеджеры могут управлять типами размещений" 
ON accommodation_types FOR ALL USING (is_admin_or_manager()) WITH CHECK (is_admin_or_manager());

-- Insert the data with proper JSON arrays
INSERT INTO accommodation_types (
  id, name_kz, name_ru, name_en, description_kz, description_ru, description_en, 
  price, features, image_url, is_active, created_at, updated_at, images, 
  category, total_quantity, available_quantity, weekday_price, weekend_price
) VALUES 
